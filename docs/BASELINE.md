...
| Rubric | `0.1.0`, spec sha256 `77e436ce18d682fa2e0608edfe681bbdb99854fe118650ac79713b7dca671400`, rubric fingerprint `18e704f2` |
...
`18e704f2` was labelled "engine fingerprint" here until 13 Sep 2026. It is not one. It is
`fingerprint(rubric)` — `core/engine.ts:900` writes `rubric_fingerprint: fingerprint(rubric)`, and
recomputing `fingerprint()` over `core/rubric.prospect.v0.1.json` returns `18e704f2` exactly.

**Nothing stamps the engine.** A read records which rubric scored it and never which code did, so a
change made in `core/engine.ts` alone — the chase key is built there from hard-coded constants at
`engine.ts:837-843`, not read from the spec — would write reads that order differently while
carrying an identical `rubric_version` and `rubric_fingerprint`. The baseline rows themselves are
safe (§3), but the provenance stamp cannot tell you that a later run used different code. Until an
engine stamp exists, an engine-only ordering change must be recorded in `docs/DECISIONS.md` by hand,
because nothing in the data will show it.

The baseline fingerprint below is a different thing from either: it is `md5` over
`account_id|effective_tier|status|cell` for all 680 rows, ordered
