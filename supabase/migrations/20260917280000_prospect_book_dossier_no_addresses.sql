-- The "names only" fix (previous migration) did not do what its comment claimed.
--
-- It stopped selecting the attendee EMAIL field, and then returned the attendee NAME — but where
-- Fathom never learned a name, the name IS the address. 332 attendee entries in pb_calls are in
-- exactly that state, so the addresses kept flowing to a page with no sign-in. Checking the shape
-- of the fix instead of its OUTPUT is how it passed: the field was gone and the leak was not.
--
-- Free text carries them too, and always did: a fact's note, a contact event's subject, a register
-- row's text, the written brief. Those are prose a person or an extractor wrote, and an address
-- inside a sentence is as published as one in a column.
--
-- So the rule moves from "do not select that field" to "no address leaves this function", applied
-- once to the whole result rather than per key — the per-key version is what just failed. Each
-- address is reduced to its LOCAL PART, which is what the 16 September layout renders anyway:
-- provenance still reads as a person or an extractor version (rule 8's audit trail is intact,
-- which is why this is not simply redaction), and nothing returned is a deliverable address.
--
-- Verify by OUTPUT, never by shape:
--   select count(*) from (select pb_dossier(account_id) j from pb_board()) d
--    where j::text ~ '[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}';   -- must be 0
...
comment on function public.pb_dossier(uuid) is
  ...
  'still identifies who or what wrote a row, so rule 8''s audit trail is intact.';
