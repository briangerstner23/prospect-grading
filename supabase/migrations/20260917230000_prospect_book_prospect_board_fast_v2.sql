-- Second attempt, and this one measures. Unnesting the array (previous migration) was necessary
-- and nowhere near sufficient: 8.1s → 7.7s. The plan showed why —
--     Unique → Index Scan on pb_reads (rows=13584, LOOPS=599)
-- pb_current_reads is a `distinct on` over every row of pb_reads, and DISTINCT ON blocks
-- predicate pushdown, so no join condition can reach inside it. The planner rebuilt the entire
-- current-reads set once per company, 599 times.
--
-- The fix is to evaluate it ONCE. `as materialized` is an optimisation fence: Postgres computes
-- the CTE a single time and hash-joins against the result. One pass over 13,584 rows instead of
-- 599 of them.
--
-- Output is unchanged — same rows, same distinct-on, same chase_rank_key order (rule 4).
