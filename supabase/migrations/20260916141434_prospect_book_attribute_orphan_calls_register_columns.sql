...
-- Applied 20260916141434 as "prospect_book_attribute_orphan_calls_register_columns" by the 16 September session, which pushed its work to its own
...
-- pb_register's columns are `text` and `made_by`, not `body`/`author`. Same class of error as the
-- other two in this function: plpgsql does not resolve column names in its body at CREATE time,
-- so a wrong one survives until the first real run. Dry-run mode never reached this statement,
-- which is exactly why the dry run passed three times while the write path was still broken.
