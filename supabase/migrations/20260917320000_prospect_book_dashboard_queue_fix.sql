-- The first version of the queue block joined the grouped counts back against the raw rows to get
-- its two scalars, which multiplied them: total read 5490 against 915 real candidates, and
-- confirmable_together read 1272 against 212. by_reason was right, which is exactly why it was
...
as $$
  ...
  qg as (select reason, count(*) as n from q group by 1)
  select jsonb_build_object(

    'queue', jsonb_build_object(
      'total', (select count(*) from q),
      'confirmable_together', (select count(*) from q where reason = 'clear — can be confirmed together'),
      'by_reason', coalesce((select jsonb_object_agg(reason, n) from qg), '{}'::jsonb)),
